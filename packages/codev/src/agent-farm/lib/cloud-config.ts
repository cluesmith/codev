...
export function readCloudConfig(): CloudConfig | null {
  ...
  try {
    ...
    throw new Error(
      `Cloud config at ${configPath} contains invalid JSON. Delete it and re-register with 'afx tower connect'.`
    );
  }
  ...
  if (missing.length > 0) {
    console.warn(
      `Cloud config at ${configPath} is missing required fields: ${missing.join(', ')}. ` +
        `Tower will operate in local-only mode. Fix with 'afx tower connect'.`
    );
    return null;
  }
