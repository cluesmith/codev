...
CREATE TABLE IF NOT EXISTS mailbox (
  ...
  interrupt_outcome TEXT,                 -- audit for the armed force: 'armed' | 'claimed' | 'claimed-degraded' | 'written-unverified' | 'degraded-written-unverified' | 'failed' | 'degraded-failed' | 'skipped-offline' | 'skipped-session-replaced' | 'skipped-contended' | 'skipped-restart'. NEVER receipt: 'claimed' only means the row was claimed before the write. No CHECK on purpose -- SQLite cannot ALTER one in, so a fresh install would diverge from an upgraded one (same reasoning as the detail column); the value set is enforced in TypeScript (MailboxInterruptOutcome)
