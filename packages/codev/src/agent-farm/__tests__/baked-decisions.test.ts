...
describe('Spec 746 Phase 1: builder-prompt baked-decisions instruction', () => {
  ...
  // Spec 1252 (Phase 4): the mirror-parity describe that lived here compared
  // the Baked Decisions paragraph across codev/ and codev-skeleton/. The
  // codev/ shadow tree is deleted — there is no second tree, so the
  // comparison became file-vs-itself and was removed as vacuous. Drift of the
  // paragraph is now impossible by construction (single owner), and its
  // presence/wording is still asserted by the grep-regression tests above.
});
