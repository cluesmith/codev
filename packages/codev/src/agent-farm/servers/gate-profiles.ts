 ...
 *
 * Only apps the spike measured have a profile (claude, codex). Everything else —
 * gemini, opencode, agy, an unknown binary, or a launch we can't identify —
 ...
 * agy terminal masquerade as claude and receive claude's (wrong) profile. agy in
 * particular can never classify clean — its `> ` marker and normal-intensity hint
 * text fit no measured profile (constraint 13) — so mapping it to claude would be
 ...
 * agent (via {@link detectHarnessFromCommand}). Wrapped launches — a builder run
 * through `.builder-start.sh` whose `command` is the shell, not the agent —
 * resolve to `null` here; the delivery wiring (Phase 4) is responsible for
 * supplying the resolved agent command for those (it already reads the launch
