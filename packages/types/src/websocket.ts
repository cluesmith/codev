 ...
 *
 * - `TOWER_KEY_HEADER`: the HTTP header carrying the key on authenticated
 *   requests. `LEGACY_WEB_KEY_HEADER` is the former name (`codev-web-key`),
 *   which the server still accepts for one release so an already-installed
 *   VS Code / Stream Deck client (bundling an older sdk) keeps working while it
 *   updates. New clients send `TOWER_KEY_HEADER`.
...
export const LEGACY_WEB_KEY_HEADER = 'codev-web-key';
