...
## Cloud dev-run defect + fix (2026-09-15)
codev-ide's cloud dev-run through /t/<tower>/ide/ found a REAL defect: Tower spawns the IDE server with --connection-token-file, but the forward never presented that token upstream → server 403s. (Repro: keyed GET 4100/ide/ → 403; + Cookie: vscode-tkn=<token> → 200 + full workbench.)
FIX (main ruling, option a): inject `vscode-tkn=<token>` into the Cookie header in buildForwardHeaders for BOTH HTTP and WS upgrade, merging with any existing Cookie, never clobbering. Token read via new readIdeConnectionToken() (~/.agent-farm/ide-connection-token). Why inject at the forward (not rely on the browser): the server's Set-Cookie is Path=/ide, which never matches the external /t/<tower>/ide/ through the relay — so the browser never holds it; the forward is the correct layer.
