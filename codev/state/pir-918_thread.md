...
Architect approved the `pr` gate; verified via `porch next` (status: complete). Merged PR #938 with `gh pr merge --merge --admin`; recorded `porch done --merged 938`. PR state MERGED, protocol complete. Fixes #918 auto-closes the issue.

Note from architect: this PR claims the command name `codev.searchBacklog`; pir-920 (editor-tab webview) must pick a different name per the cross-PR naming note in #920's body. Ready for cleanup.
