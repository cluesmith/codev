...
- [ ] The panel header renders four mode pills (Document Review, Code Review, Builder Inspector, Attention). The pill for the current mode is visually distinct. Modes inapplicable to the current context are visibly greyed rather than hidden, and remain clickable for navigation.
- [ ] **Mode pills are transient navigation, not override.** Clicking an applicable pill (or drilling into a builder row in a summary list) navigates the panel without touching the editor. This manual selection is discarded on the next active-surface change, at which point contextual resolution resumes. It is never persisted.
...
- [ ] The panel exposes a render target per mode and per level (summary / detail) but does **not** implement mode content — it ships placeholder bodies plus the orchestration that switches between them.
...
- **No pinning / no persisted panel state (architect decision, revises issue #1049).** The panel is purely contextual: it follows the active surface and never locks. The mode pills are a transient navigation affordance whose selection is discarded on any active-surface change and is never written to disk. This supersedes issue #1049's pinning language (the pin icon, the `codev.contextualPanel.pinnedMode` persistence key, "power users can pin a mode to lock the panel," and the "pin Attention, switch editors, stays on Attention" dev-approval step); the issue body should be amended to match.
...
| Auto-switch yanks the panel away while the user is mid-typing a comment (no pin to hold it) | Medium | Medium | Define the sticky-input contract (`isEditing` defer signal) at the plan gate; hold re-resolution until input settles. |
