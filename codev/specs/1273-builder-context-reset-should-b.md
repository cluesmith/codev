...
| How much porch context must re-orientation carry? | "porch-strict lanes must re-inject phase context like `--resume` does" (issue + architect). |
...
3. **Request the save.** Send a templated save-state request carrying a fresh nonce, the exact
   target path, and a cold-reader content checklist.
...
- [ ] Unit tests cover the reset state machine's ordering invariants (R1, R2, R3) directly — each
      invariant has a test that fails if the ordering is inverted.
...
- Scope is bounded to the three deliverables above. Related prior art (the `/exit` + `spawn --resume`
  boundary-recycle pattern, codev#1260's target-by-convention discussion) informs the design but is
...
- [ ] What is the right minimum-substance threshold for the state file? The one verified example was
      203 lines. A byte threshold with a flag override is proposed; the default value should be set
      in the plan and justified.
- [ ] Can `/clear` be positively confirmed from terminal output across Claude Code versions, or must
      ...
      the ordering.
- [ ] Should the re-orientation's long form live in a worktree file that the builder is instructed to
      read, or be inlined into the message? Proposed: compact inline frame (guaranteed to arrive)
      **plus** a file pointer for the long form — belt and braces, since the inline frame alone
      satisfies R3.
...
- Prior art: the `/exit` + `spawn --resume` boundary-recycle pattern; codev#1260 target-by-convention.
