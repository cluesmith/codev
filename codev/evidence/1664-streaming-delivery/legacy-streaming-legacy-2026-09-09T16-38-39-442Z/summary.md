# Issue #1664 — streaming delivery acceptance (legacy)

- harness: `claude` 2.1.266 (Claude Code), 120x40, 20 trials
- gate: the RETIRED whole-screen output settle (--legacy-gate)
...
- longest output gap seen while a row waited: 341 ms (the retired whole-screen settle needed 250 ms to deliver at all)
- wait to delivery: mean 2709 ms, median 2505 ms, max 6432 ms
...
- repaints the recipient emitted while the row waited: median 27

## Hold verdicts observed

- `busy (legacy whole-screen settle)`
