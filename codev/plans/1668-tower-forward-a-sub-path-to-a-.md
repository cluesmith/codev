...
4. `afx ide start | stop | status` lifecycle where **Tower spawns/manages** the single local server. `start` passes the binary path + port to Tower, which spawns it and records `{prefix, port, serverPath, pid}` in a Tower-managed `~/.agent-farm/ide-server.json` file, so Tower can forward, reconcile on restart, and respawn a dead server. Live PID is discovered by port. **No config file and no database change** (Decisions §D3).
5. **gzip/brotli is deferred** (Decisions §D5) — not built in this lane.
6. No config surface / no config file / no DB change: the fixed prefix is a `/ide/` constant, the port and binary path are `afx ide start` flags, and the runtime record lives in Tower's `~/.agent-farm/ide-server.json` (the `cloud-config.json` pattern). `/api/tunnel/*` stays local-only as today.
...
- **§D3 — No config file, no DB change. State lives in a Tower-managed `~/.agent-farm/ide-server.json` record.** *Decided (Amr, at the gate).* A single server whose only no-default input is the binary path needs no config file. `afx ide start --server-path <bin> [--port] [--default-folder]` supplies those as flags; Tower spawns the server (§4) and writes `{prefix, port, serverPath, pid}` to `~/.agent-farm/ide-server.json`. On restart Tower reads the record and *reconciles* (re-adopt the live server by port; respawn if dead) instead of orphaning. **No database change.** The prefix is a fixed `/ide/` constant (not a flag); the port defaults (8200).
  - *Why a Tower-managed record file, not a config file and not `global.db`:* (1) it's **not** the hand-edited config file we rejected — Tower *writes* it, exactly as it already writes `~/.agent-farm/cloud-config.json` and `machine-id`; that's the established home for Tower's own singleton runtime facts. (2) It's **not** a `global.db` schema change — a single `{port, serverPath, pid}` record is a singleton, the same shape as `cloud-config.json`, so it belongs beside those files, not in a relational table; keeping it out of `global.db` means no migration. (3) A per-repo `.codev/config.json` block was rejected as ambiguous for a single global server; a machine-global `~/.codev` config would be hand-edited for what is really runtime state.
...
- **§D4 — PID-by-port discovery** (builder discretion): `afx ide stop`/`status` and Tower's restart reconcile find the live server via `getProcessesOnPort(port)`; the `~/.agent-farm/ide-server.json` record supplies the port and `serverPath` for respawn. `pid` in the record is a convenience/cross-check, authoritative liveness is the port probe.
...
- Resolve the target `127.0.0.1:<port>` from the `~/.agent-farm/ide-server.json` record (single server; §D3). No record / no live server ⇒ 404/502.
...
### 4. `afx ide` lifecycle (Tower spawns; `~/.agent-farm/ide-server.json` record — §D3)

The CLI is **commander**-based (`agent-farm/cli.ts`, built in `runAgentFarm`). Add an `ide` command group next to the `tower` group (model at `cli.ts:941-943`), with `start`/`stop`/`status` subcommands, delegating to a new `commands/ide.ts` (modeled on `commands/tower.ts`). There is **one** server (§D1); the prefix is the fixed `/ide/` constant and the port defaults to 8200.

- `afx ide start --server-path <bin> [--port 8200] [--default-folder <path>]` asks **Tower to spawn** the server (a key-authed local `POST /api/ide` call; Tower is the spawner so auto-start/respawn reuse the same path). Tower spawns detached — mirroring `towerStart` (`commands/tower.ts:167`): `spawn(bin, args, { detached: true, stdio: 'ignore' })` + `unref()` — with args `--host 127.0.0.1 --port <p> --server-base-path /ide --connection-token-file <tower-owned file> --accept-server-license-terms` (+ `--default-folder`), then writes `{prefix, port, serverPath, pid}` to `~/.agent-farm/ide-server.json`. `--server-path` is required (no default — the real build lives in a private fork); missing ⇒ clear error. Idempotent: no-op if already live on the port.
- `afx ide stop` (a `DELETE /api/ide` call) → Tower reads the record, discovers the live pid **by port** (`getProcessesOnPort` from `utils/port.ts:24`), SIGTERMs it (poll `process.kill(pid,0)` → SIGKILL escalation, as `towerStop` does at `tower.ts:355-383`), and deletes the record.
- `afx ide status` (a `GET /api/ide` call) → reports prefix + port + PID + liveness from the record + port probe. **Doc note:** each open browser window spawns its own extension host (~100–300 MB); N windows = N hosts — surfaced here so the memory cost is visible.
- The `/api/ide` endpoints are **local-only management** routes — blocked from the tunnel like `/api/tunnel/*` (they spawn/kill a process) and key-authed like every other route.
- **Restart reconcile / respawn (§D4):** on boot Tower reads `~/.agent-farm/ide-server.json`; if a server is still live on the port it re-adopts it (no orphan), and if the port is dead it best-effort respawns from the recorded `serverPath` (bounded readiness wait before the first forward, like `towerStart`, so the first `/ide/` hit doesn't 502 on startup latency). Wire this into `servers/tower-server.ts` boot beside the other background services (`codev-config-watcher.ts`, `tower-cron.ts`). Auto-start on a `/ide/` hit with a record but no live server reuses the same respawn path. The connection-token-file is Tower-owned; that token is the IDE server's own inner layer, **not** a substitute for the key check.
...
- `packages/codev/src/agent-farm/lib/ide-record.ts` — **new.** Read/write/delete the `~/.agent-farm/ide-server.json` record (`{prefix, port, serverPath, pid}`), modeled on `lib/cloud-config.ts` (`mkdirSync 0700` + `writeFileSync`). **No `global.db` change.**
- `packages/codev/src/agent-farm/servers/tower-ide.ts` (or in `tower-routes.ts`) — **new.** Tower-side spawn / kill / reconcile / respawn (reused by the endpoint, boot, and auto-start) + the key-authed **local-only** `POST/DELETE/GET /api/ide` endpoint the `afx ide` commands call. Blocked from the tunnel like `/api/tunnel/*`.
- `packages/codev/src/agent-farm/servers/tower-server.ts` — boot-time reconcile/respawn wiring (beside the other background services); the IDE launcher lives in `servers/` for `serversDir` resolution.
...
- Lifecycle: `afx ide start` spawns + writes `~/.agent-farm/ide-server.json`; `afx ide status` reports port+PID by discovery; `afx ide stop` tears down + deletes the record; a simulated Tower restart reads the record, re-adopts a live server by port (no orphan), and respawns a dead one.
...
- §D3 config — **no config file, no DB change**; `afx ide start` flags + a Tower-managed `~/.agent-farm/ide-server.json` record.
