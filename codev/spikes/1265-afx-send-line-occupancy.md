...
**Date**: 2026-07-30 (updated 2026-07-31 after review rounds 2–5)
...
2. **Drain triggers**: (a) the provably-clean convergence event that clears `dirty` (the same event that fires E's flush); (b) the periodic poll as backstop; (c) recovery convergence after a Tower restart (rows are already persisted; constraint 1's recovered-dirty state converges first, then drains). Drain is oldest-first and runs **ahead of** younger `SendBuffer` entries for the same target, preserving per-target arrival order.
...
5. **Sender semantics**: `afx send` returns `held` (with the mailbox id) vs `delivered` (constraint 6c), so humans, porch, and cron can distinguish escalation from delivery.
...
3. **Max-age path, multi-line drafts: H** — as a write-queue transaction in **divert mode** (user frames diverted for the maneuver's duration, appended after replay, fail-open timeout): per-line clear, per-app atomic inject **without the trailing `^Y`**, verbatim replay. The replay is the sole restore mechanism on this path.
...
**Medium-to-Large, phased** (~1130–1630 LOC total): DraftTracker with raw replay log + cursor-aware line model, tapped at `PtySession.write()` (~250), submit-detector (~80), defer/flush rewiring incl. flush-after-write ordering (~120), per-session write transaction queue with user-frame divert mode (~140 — replaces the round-3 "input gate ~60"; it also serializes writer-vs-writer, constraint 7), dirty-state machine + restart-recovery dirty (~110), delivery matrix + H maneuver in message-write (~150), K mailbox — DB table/migration, inbox routes, surfacing, `held` send semantics, drain scheduling per the round-5 semantics, persist-on-shutdown, cron rerouting (~350–450), tests (~400+, incl. a keystroke-sequence unit suite derived from the classification table and the POC's asserted scenarios). Hooks all sit at existing choke points (`PtySession.write`, `SendBuffer`, `handleSend`) — no architectural change — but round 3 (gate/dirty/recovery), round 4 (cursor model, K-as-mailbox), and round 5 (write queue, drain semantics) push the total past Medium if shipped monolithically. **Phase it**: (1) A+E core + write-tap + cursor model + write transaction queue — kills the common-case corruption AND today's concurrent-sender blob (`w1a`) on its own; (2) H/matrix under the queue's divert mode; (3) K mailbox + drain + cron rerouting + restart recovery. Each phase is an independently shippable Medium.

## POC methodology (revised after round-4 review)

A reviewer rerunning `exp-i6-gating.cjs` got `i6a-gated-clean=false` with exit 0 — a fair hit on the original methodology: verdicts were printed, not asserted, and that particular false-FAIL came from a cross-demo contamination (i6b's dead-API retry queue + claude's ESC-restores-queued-messages behavior polluted i6a's reference draft, while the gated run itself was correct against its own rebuilt draft). Fixes applied:

- **Two evidence tiers.** `exp0`–`exp-i4` are *exploratory* (observation prints; they discovered the behaviors). `exp-a3`, `exp-i5`, `exp-i6` are *regression-grade*: every claim is an `ASSERT` with an expected value, any failure exits non-zero, and each demo runs in its **own fresh TUI session** with **per-demo baselines** (no shared state across demos).
...
None in repo test suites (the spike ran none). One POC flake existed and was root-caused: the original single-session `exp-i6` reused a claude session across demos, and queued-message restore contaminated a later demo's baseline — fixed by per-demo session isolation + per-demo baselines (see POC methodology).
