...
**Medium-to-Large, phased** (~1050–1550 LOC total): DraftTracker with raw replay log + cursor-aware line model, tapped at `PtySession.write()` (~250), submit-detector (~80), defer/flush rewiring incl. flush-after-write ordering (~120), input gate (~60), dirty-state machine + restart-recovery dirty (~110), delivery matrix + H maneuver in message-write (~150), K mailbox — DB table/migration, inbox routes, surfacing, `held` send semantics, persist-on-shutdown, cron rerouting (~350–450), tests (~400+, incl. a keystroke-sequence unit suite derived from the classification table and the POC's asserted scenarios). Hooks all sit at existing choke points (`PtySession.write`, `SendBuffer`, `handleSend`) — no architectural change — but round 3 (gate/dirty/recovery) and round 4 (cursor model, K-as-mailbox) push the total past Medium if shipped monolithically. **Phase it**: (1) A+E core + write-tap + cursor model — kills the common-case corruption on its own; (2) H/matrix + input gate; (3) K mailbox + cron rerouting + restart recovery. Each phase is an independently shippable Medium.
