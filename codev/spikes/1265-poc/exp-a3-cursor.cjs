 ...
 *
 * Asserted probes (exit 1 on any failure):
...
(async () => {
  const d = new TuiDriver(which, [], { label: `expA3-${which}` });
  ...
  d.send(KEYS.CTRL_E); await sleep(80); d.send(KEYS.CTRL_U); await sleep(200);

  // a3b: mid-line Backspace
  await d.type('uvwxyz');
  ...
  // a3c: ^E then ^U from a mid-line cursor fully clears (maneuver safety)
  await d.type('uvwxyz');
  for (let i = 0; i < 3; i++) { d.send(KEYS.LEFT); await sleep(70); }
