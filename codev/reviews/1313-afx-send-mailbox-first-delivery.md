...
- **Tests**: full unit suite **4540 passing** (48 pre-existing skips, 0 failures) at last green (2026-08-06, after the ghost-cursor fix). New suites: `mailbox`, `render-gate`, `send-delivery`, `send-mailbox-repro`, `write-queue`, `cron-delivery`, `inbox-cli`, `inbox-routes`, `spec-1313-migration`, `spec-1313-registry-resolve`, `spec-1313-resolve-agent-for-session`, `send-architect-identity`, `pty-session-delivery-signals`, plus dashboard `HeldCountBadge`, VSCode `mailbox-indicators`/`mailbox-escalation-toast`, and the Playwright `spec-1313-held-count-indicator` e2e.
...
- **Ghost-cursor exemption — accepted narrow residual + a deferred liveness diagnostic** (2026-08-06). The inverse-gated cursor-cell exemption carries one residual: a **1-char draft with the cursor relocated onto its only char** (Home/Left) inside the exact delivery window classifies CLEAN — identical to the plain cursor-skip option's residual, and impossible for any multi-char draft (every other cell still counts). Accepted because it is vanishingly rare and strictly fail-safe-adjacent: measured, claude leaves the inverse block cursor on trailing whitespace, so a real 1-char draft with the cursor *at end* stays `busy`, and typed chars are never inverse-rendered. Separately, the finding's optional hardening — flag a sustained `user-text` hold whose counted-cell set is exactly {the cursor cell} as chrome in the classifier-stuck liveness net — is **deferred**: the exemption stops this drift class from producing a hold at all, so it no longer manifests; the net-level diagnostic would be belt-and-suspenders.
