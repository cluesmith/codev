...
- `afx send` is mailbox-first (Spec 1313): persist to global.db first, then deliver only onto a render-gate-verified empty prompt. Any new message writer routes through the mailbox+gate — never write a PTY directly, never force-inject. Response: `delivered` | `held`+reason. Every message writer also takes the per-terminal `submitToSession` lock (#1365); lock order is per-agent → per-terminal, never the reverse. For an app MEASURED to queue input mid-turn (`GateProfile.queuesInputMidTurn` — claude, codex), only a HUMAN at the line holds mail (#1664): the gate waits on the COMPOSER REGION's stability, not the recipient's output (`busy:composer-redraw` is the residual). Every other app keeps the #1573 whole-screen settle — the capability is measured per app, never inherited. The INPUT half is untouched (#1473): `PtySession.write()`'s `inputSeq` + a 300ms settle hold a keystroke (`busy:recent-input`), with terminal REPLIES (DA/DSR/CPR/…) stripped from that signal server-side (`terminal-replies.ts`), signal-only, never from the PTY write.
