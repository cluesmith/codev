...
- `afx send` is mailbox-first (Spec 1313): persist to global.db first, then deliver only onto a render-gate-verified empty prompt. Any new message writer routes through the mailbox+gate — never write a PTY directly, never force-inject. Response: `delivered` | `held`+reason. Every message writer also takes the per-terminal `submitToSession` lock (#1365); lock order is per-agent → per-terminal, never the reverse. Only a HUMAN at the line holds mail (#1664): the gate waits on the COMPOSER REGION's stability (`composerRegionFingerprint`, two samples ≥250ms apart), never on the recipient's output — a working agent repaints ~8x/sec and queues input received mid-turn, so a spinner is not a reason to wait (`busy:composer-redraw` names the residual). The INPUT half is untouched (#1473): `PtySession.write()`'s `inputSeq` + a 300ms settle hold a keystroke (`busy:recent-input`), with terminal REPLIES (DA/DSR/CPR/…) stripped from that signal server-side (`terminal-replies.ts`), signal-only, never from the PTY write.
