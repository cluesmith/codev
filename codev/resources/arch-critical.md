...
- `afx send` is mailbox-first (Spec 1313): persist to global.db first, then deliver only onto a render-gate-verified empty prompt. Any new message writer routes through the mailbox+gate — never write a PTY directly, and never add a force the system decides on its own. The ONLY ungated writes are per-message and sender-requested: `--interrupt` (now) and `--interrupt-after <s>` (only if still held at its deadline, #1481), both through one shared interrupt writer. Response: `delivered` | `held`+reason; a force outcome is audit, never receipt. Every message writer takes the per-terminal `submitToSession` lock (#1365) plus the per-row write-ownership token (#1481) before its first byte; lock order is per-agent → per-terminal, never the reverse.
