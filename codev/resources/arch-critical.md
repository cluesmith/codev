...
- `afx send` is mailbox-first (Spec 1313): persist to global.db first, then deliver only onto a render-gate-verified empty prompt. Any new message writer routes through the mailbox+gate — never write a PTY directly, never force-inject. Response: `delivered` | `held`+reason. Every message writer also takes the per-terminal `submitToSession` lock (#1365); lock order is per-agent → per-terminal, never the reverse. Only a HUMAN at the line holds mail (#1664): for an app carrying `GateProfile.queuesInputMidTurn` (all three today — claude/codex measured, agy by owner ruling) the gate waits on the COMPOSER REGION's stability, not the recipient's output (`busy:composer-redraw` is the residual). An app without it keeps the #1573 whole-screen settle. The INPUT half is untouched (#1473): `PtySession.write()`'s `inputSeq` + a 300ms settle hold a keystroke (`busy:recent-input`), with terminal REPLIES (DA/DSR/CPR/…) stripped from that signal server-side (`terminal-replies.ts`), signal-only, never from the PTY write.
