...
- `afx send` is mailbox-first (Spec 1313): persist to global.db first, then deliver only onto a render-gate-verified empty prompt. Any new message writer routes through the mailbox+gate — never write a PTY directly, never force-inject. Response: `delivered` | `held`+reason. Every message writer also takes the per-terminal `submitToSession` lock (#1365); lock order is per-agent → per-terminal, never the reverse.
