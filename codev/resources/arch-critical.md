...
- `afx send` is mailbox-first (Spec 1313): persist to global.db first, then deliver only onto a render-gate-verified empty prompt. Any new message writer routes through the mailbox+gate — never write a PTY directly, never force-inject. Response: `delivered` | `held`+reason. Every message writer also takes the per-terminal `submitToSession` lock (#1365); lock order is per-agent → per-terminal, never the reverse. The gate samples INPUT as well as output (#1473): `PtySession.write()` carries an `inputSeq` counter folded into the change token plus a 300ms input-settle, so a keystroke holds the message (`busy:recent-input`) — and terminal REPLIES (DA/DSR/CPR/…) are stripped from that signal server-side (`terminal-replies.ts`), signal-only, never from the PTY write.
