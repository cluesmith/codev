...
**Location**: `db/mailbox.ts`, `servers/render-gate.ts`, `servers/gate-profiles.ts`, `servers/write-queue.ts`, `servers/session-submit.ts`, `servers/message-write.ts`, `servers/mailbox-delivery.ts`, `servers/mailbox-interrupt.ts`, `servers/row-write-ownership.ts`, `servers/mailbox-wiring.ts`, `servers/cron-delivery.ts`, `commands/send.ts`, `commands/inbox.ts`, `terminal/pty-session.ts`

Spec 1313 replaced Spec 403's in-memory, timer-based, force-flushing `SendBuffer` (deleted) with a **mailbox-first** pipeline. The governing invariant: a message body is **only ever written to a prompt a headless-terminal render-gate proves is empty**, so it can never fuse with a draft, a menu, a dialog, or a wrapper screen — corruption is eliminated *by construction*, not detect-and-repair. **The system never forces on its own**: no timeout, valve, or fallback the system applies unasked ever writes onto a non-clean screen. The only two paths that write ungated are per-message and requested by the sender — `--interrupt` (now) and `--interrupt-after <s>` (only if the row is still held at its deadline; Issue #1481) — and both go through the one shared interrupt writer under the same per-terminal lock as a gated delivery. Only the timed force also takes the per-row ownership token, because only it can race a gated write for the *same* row; immediate `--interrupt` marks its row delivered before writing, so no gated pass can still be holding it. Any new automated message writer MUST route through this mailbox+gate — never write a PTY directly, and never add a system-decided force.
...
7. **Two bypasses, both per-message and both opt-in.** `--interrupt` forces **now** — an explicit, deliberate sender action (interrupts the agent, writes without a gate check). `--interrupt-after <s>` (Issue #1481) is the **bounded-patience** form: the row is ordinary held mail, eligible for the gate from the moment it is persisted, and only *if it is still held* at `interrupt_at` does a coordinator escalate it to the **same** forced write. Both are per-message commands, not timeouts or valves the system applies on its own, so neither weakens the no-force-path invariant — what the sender opts into is exactly one forced write for exactly one row. `noEnter` sends are gate-checked staging (write text, no Enter) → report `delivered`.

   *Bounded patience mechanics (`servers/mailbox-interrupt.ts`, `servers/row-write-ownership.ts`).* Four `mailbox` columns carry it (migration v19): `interrupt_at`, `interrupt_claimed_at`, `interrupt_outcome`, `interrupt_prior_partial`. `MailboxInterruptCoordinator` (owned by `mailbox-wiring.ts`'s start/stop lifecycle) arms an absolute-deadline timer at persist time — **before** the first gated attempt, so a slow classify cannot extend the operator's budget. At the deadline it re-resolves the agent's live session, re-checks row/lifecycle/session identity **inside** the per-terminal submission lock, atomically claims `held → delivered` in one guarded UPDATE, and only then calls the shared `writeInterruptToSession` (`message-write.ts`) that `--interrupt` itself uses: `^C`, a 100 ms settle, the ungated body, Enter unless `noEnter`. Claim-before-bytes matches immediate `--interrupt`'s loss-over-duplicate choice, so `claimed`/`claimed-degraded` means **unknown write outcome, never receipt**; the vocabulary keeps `written-unverified`, `failed` and `skipped-*` distinct rather than collapsing them into a lossy "done". **A force emits the delivery feed event only when a body actually went out**: `failed`/`degraded-failed` (the terminal rejected the write) report through the outcome notification and the log alone, because the feed frame carries the outcome as *optional* metadata a consumer may ignore, and a body on the feed reads as receipt. Both floating entry points (the deadline timer and the row-ownership continuation) run behind a boundary that cannot reject — `tower-server.ts` exits the process on `unhandledRejection`, so an unguarded DB error in an escalation would take Tower down fleet-wide; the boundary logs, records `skipped-error`, and leaves the body as ordinary held mail.

   *Why a per-ROW ownership token exists.* The per-terminal lock is not sufficient here, because a gated delivery's row still reads `held` while its paced write is on the wire, and the operator ceiling may legitimately bypass a delivery. `row-write-ownership.ts` adds a synchronous, non-blocking token that the two paths which can write the SAME row — a gated delivery and the timed force — take immediately before their first byte, so "is someone writing this row right now?" is never inferred from `status`. The force declines (writing nothing, claiming nothing, counting no bypass) when a gated writer owns the row, and re-enters once that attempt's outcome is committed. Lock order is unchanged — per-agent → per-terminal — and the force never acquires the per-agent serializer, so a queued escalation cannot stall the sequential drainer.

   *Force authority is scoped to the Tower lifetime.* A restart sweep (before any writer starts) retires leftover `armed` rows as `skipped-restart`, keeping the body and the audit but never rearming; an offline or replaced session at the deadline records `skipped-offline` / `skipped-session-replaced`. The body stays ordinary held mail in every skip. The deliberate consequence: a restart cannot surprise an unrelated later turn with an interrupt nobody is still waiting for. `interrupt_prior_partial` is separate audit — a *possibly* partial earlier ordinary write keeps the force armed (an unresolved attempt does not cancel the escalation the operator asked for) and surfaces the duplicate risk in `afx inbox` rather than pretending exactly-once.

   *Alarm interaction.* An **armed, pre-deadline** row is excluded from starvation aggregation and owner-notice membership only — it is "will self-resolve", not stuck. Ordinary held mail for the same agent still alarms, and after the deadline (or after any skip) the row participates normally, on an escalation clock started at `max(created_at, not_before, interrupt_at)`. Held counts and inbox visibility are unchanged: bounded-patience mail is held mail, not scheduled mail.
