...
- **Public allowlist is deliberately narrow**: `/health`, `/api/version`, the dashboard shell and its static assets — everything under `api/` and `ws/` requires the key. `BRIDGE_MODE` refuses to start without a key at all
