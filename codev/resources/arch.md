...
9. **Tower API Authentication**: Tower's local HTTP + WebSocket API enforces request authentication (advisory GHSA-xvjp-7748-v88v). Every route outside the narrow public-route allowlist (`isPublicRoute` in `agent-farm/utils/server-utils.ts`) requires the shared local key (`~/.agent-farm/local-key`), sent as the `codev-tower-key` HTTP header or a `Sec-WebSocket-Protocol` subprotocol, and fails closed with 401 (the server also accepts the legacy `codev-web-key` header for one release). Any new Tower route must decide public-vs-keyed — a wrong allowlist entry either breaks a pre-auth path (health/version probes, the served HTML shells + static assets, the cloud OAuth callback redirect) or exposes a data route. The key is delivered to browser shells via same-origin serve-time injection; those shell responses omit `Access-Control-Allow-Origin` so the injected key is not cross-origin readable.

10. **Terminal dimensions must be earned, not assumed**: `PtySession` tracks *requested* dims separately from *applied* dims, and commits `cols`/`rows` — and re-wraps the render gate's classification mirror — only after the resize is confirmed to have reached the process. A resize that the shellper drops leaves Tower's dims where they were and sets an *outstanding* flag; the REST resize routes answer **409 `RESIZE_DROPPED`** (404 stays reserved for an unknown session id) rather than 200-echoing the requested dims. On attach, the shellper's WELCOME geometry wins over Tower's belief. This is load-bearing for `afx send`: the render gate classifies a headless mirror of the screen, and its verdict is only meaningful if that mirror wraps text the way the real TUI does — on a committed 139x63 fixture, classifying at 135 or 131 columns yields `busy:no-region-end`, a hold that never clears. Never assign dims before confirming the resize landed, and never discard the boolean that says it did not.

11. **Every write into a `PtySession` counts as human input unless it explicitly opts out** (Issue #1473): `write(data, origin)` defaults `origin` to `'external'`, and that default is the invariant, not a convenience. The delivery gate's input signal is recorded inside `write()` rather than at each call site precisely so a new writer is counted *by default* and a forgotten call site fails safe (an extra hold) instead of silently reopening the race. `'delivery'` is the only opt-out and is correct for exactly one thing: bytes the gate itself just authorised. Anything else that reaches a PTY — the REST write route, the terminal WebSocket, `--interrupt`'s `^C`, an escape write, a future client — is foreign input and must keep the default. Reviewing a new `write()` call means asking "did the gate authorise these bytes?"; if the answer is anything but yes, passing `'delivery'` is a defect, and one that no gate test will catch because gate tests supply their own sessions.
...
**Listing a builder and opening one read different sources, coupled by a NAME (#1473).** The Agents view lists from `GET /api/overview`, which is filesystem-derived: `discoverBuilders` does a `readdirSync` of `<workspace>/.builders/`, and each row's **`id` is the directory name verbatim** while its `roleId` is `worktreeNameToRoleId(dirName)` (`overview.ts:475-512`, e.g. `pir-1473` → `builder-pir-1473`). Clicking that row hands the same **`b.id`** (`views/builders.ts:439`) to `openBuilderByRoleOrId` → `resolveBuilderTerminal` → `resolveAgentName` (`agent-names.ts:39-60`), which matches it against **`/api/state`**'s ids — sourced from Tower's live terminal registry — by exact or **tail** match. So the directory name must satisfy *both*: derive the registered roleId, and exact- or tail-match it. A name that satisfies only the first (`pir-1473-probe` → roleId `builder-pir-1473`, which does not end with `-pir-1473-probe`) lists fine and resolves `missing` on click, surfacing as "…terminal isn't available yet" against a row sitting right there. Nothing else documents this coupling, and verifying `/api/overview` does not test it.

**Contextual bottom panel (#1049, `src/contextual-panel/`).** The extension's first `WebviewViewProvider` (all other panel/sidebar views are trees). It splits a **pure, `vscode`-free core** — `resolver.ts` (`resolveMode`), `surface-context.ts` (`deriveSurfaceContext`/`surfaceKey`), `pills.ts`, `messages.ts` — from the `vscode`-touching host: `surface-reader.ts` (classifies the active tab, reads live state) and `panel-provider.ts` (subscribes the trigger set, posts a `ModeDescriptor`). The core is source-scan-tested to stay pure. Two VS Code gotchas the panel works around: the multi-file diff (`vscode.changes`) has **no typed tab input** even in 1.105 (classify via `activeTextEditor` gated by the active tab's type); and with `retainContextWhenHidden` the provider re-posts its cached descriptor on `onDidChangeVisibility` because `resolveWebviewView` does not re-fire. The panel is purely contextual — no pinning, no persisted state. **Attention body (#1553):** the fallback view's roll-up (builders at gates, builders waiting on input, held mail, queued feedback, honest empty state) is projected by `deriveAttention(OverviewData)` — a **pure cross-client helper co-located with `isIdleWaiting` in `@cluesmith/codev-sdk/builder-helpers`**, not extension-local, so a future dashboard/Stream Deck attention view shares one definition and can't drift. The projection rides beside the descriptor on the `RenderMessage` (keeping `ModeDescriptor`/the resolver pure), and the provider re-posts it on `overviewCache.onDidChange` only while Attention is showing. Colors reuse the sidebar's exact `ThemeColor` tokens for the same states (`notificationsWarningIcon` blocked, `notificationsInfoIcon` idle).
...
- **Panel view container (#812)**: The extension contributes view containers to **two** locations — `activitybar.codev` (the 7-section sidebar) and `panel.codevPanel` (bottom panel, wide-short geometry). The panel hosts views whose shape suits a wide layout. It carries `codev.dev` (a `TreeDataProvider`, #921) and the contextual `codev.contextualPanel` (a `WebviewViewProvider`, #1049) — VS Code lets tree and webview views coexist in one container. **#1049 retired the original `codev.panelContainerEmpty` placeholder view + its dead context-key flip** (it could never render, since `codev.dev` seeded the key `false` unconditionally); the follow-ups once tracked as sibling tabs (#813/#814/#815) are rescoped to render into the contextual panel's modes, not their own tabs. VS Code gives no control over panel-tab *position* (it lands last, in the `…` overflow), so the extension does a one-time, globalState-guarded reveal (`workbench.view.extension.codevPanel`) on first activation for discoverability.
...
2. **Gate before every write.** The render-gate (`render-gate.ts`) classifies the session's **persistent bounded headless-screen mirror** (`SessionScreen`, fed the session's output byte-for-byte from birth on the live path — its current viewport IS the live screen) and applies a per-app classifier profile (`gate-profiles.ts`: **claude** & **codex** use a dim-placeholder rule; **agy** uses a color-keyed `placeholderFgPalette` rule — see below). For agy, "marker present" is more than a text match (#1474): `> ` also renders agy's slash-menu selection cursor, its dialog options and its per-turn transcript echo, so the profile additionally requires the marker row to hold the **cursor** and the marker glyph to render in the profile's palette (`markerRequiresCursorRow` / `markerFgPalette`) — a row failing either is not a marker, so drift lands on `no-composer-marker` → HOLD. Clean (marker present AND a *positively-bounded* composer region — a rule/status line below the marker, never a scan to the screen bottom — with zero normal-intensity cells) → deliver; else → keep holding. One measured exemption (Spec 1313, 2026-08-06): claude paints a **suggested-command ghost** into an *idle* composer when its own last reply mentioned a runnable command, and the ghost's first char doubles as the software block cursor — SGR-7 **inverse at normal intensity** over a SGR-2-dim tail. The classifier exempts exactly that one cell (inverse + non-dim, at the headless cursor, with a **non-empty** dim tail as positive ghost evidence — `isGhostCursorCell`), so an idle ghost no longer classifies `busy` forever and strands mail to an unattended agent; it is deliberately not a blanket inverse skip (an inverse selection over a real draft keeps every other cell counted), and a real draft never trips it because claude never inverse-renders typed text (the block cursor rests on trailing whitespace) and a lone inverse cell with an empty tail — a 1-char draft with the cursor on its only char — stays `busy` (fail-toward-hold). codex renders its own ghost wholly dim, so the dim rule already covers it. **Why a persistent mirror, not a whole-ring re-render** (Spec 1313 round 2): the gate originally rebuilt the screen each check by replaying the whole output ring through a throwaway terminal. But #1205 caps the ring's newline-free `partial` at 2 MiB (`trimPartial` halves to ~1 MiB), and a claude/codex alt-screen frame is one giant partial — so once a busy long-lived agent's frame crossed the cap the ring handed the gate a **torn** front (dropped composer marker/rule → permanent `busy`), resurrecting the over-ceiling outage for exactly the busiest agents. A bounded terminal mirror needs only the live byte stream, so the cap is irrelevant, the live-ring tear is gone, each classify is O(viewport) not O(ring size), and the whole-render era's unbounded-`partial` OOM risk (#1047) is closed. The mirror is fed at `PtySession`'s single output chokepoint (`onPtyData` + the `attachShellper` replay seed), in lockstep with `RingBuffer.bytesWritten` — the **monotone** gate change-token (cumulative output bytes; the old `currentSeq:partialBytes` pair fell on a trim and could alias a stale verdict). The drainer still memoizes each verdict against the live session + that token so a static held screen skips re-classify; the cost-aware backstop backoff the whole-render era needed is retired (a viewport classify is cheap regardless of history). The throwaway-terminal `classifyScreen(snapshot)` path survives only for the fixture suite. (Caveat: on adopt/reconnect after a Tower restart the mirror is seeded from the FULL shellper replay — ≤8 MB wire cap; only the *ring* seed is `capRingSeed`-trimmed to 1 MiB (PIR #1354) — so a long-lived alt-screen frame is born torn only when its coherent start predates the shellper's whole retention; that residual case classifies not-clean → the gate HOLDS, fail-safe, and self-heals on the next repaint. Startup cost is bounded by draining each >1 MiB seed's parse before adopting the next session, measured ~58 ms/session at 8 MB. Residual tracked as #1361.)
2b. **The gate's INPUT half (Issue #1473).** Until #1473 the gate proved emptiness from **output** alone — `RingBuffer.bytesWritten` (the monotone change token) and `lastDataAt` (the output settle) — so it saw the app repaint but was blind to the human. Two races survived: a keystroke landing *after* the token sample (the token counted output, which a keystroke does not move, so both samples agreed and the write went onto a line someone had started typing on), and a keystroke landing *just before* the sample and not yet echoed (both samples agree **correctly**; no counter can catch it, only a clock). `PtySession.write()` — the single funnel every writer that reaches the PTY *through Tower* passes through; `afx attach` writes the shellper socket directly and is the one exception (see 5) — therefore carries a monotone **`inputSeq`** counter and a **`lastInputAt`** timestamp. `inputSeq` is folded into the change token (closing the first race), and `INPUT_SETTLE_BEFORE_WRITE_MS = 300` sits beside the 250 ms output settle (bounding the second). **Issue #1664 retired the OUTPUT half of both guards, for apps measured to tolerate it.** `bytesWritten` and `lastDataAt` describe the WHOLE SCREEN, and a working agent repaints its spinner and streaming transcript a **median of 8 times a second** — mean interval 122 ms, and only 3 of 728 observed seconds offered a 250 ms gap — while its composer region stays byte-identical for runs of **107 s, 60 s, 304 s, 81 s and 149 s** (1242 whole-screen changes against 4 region changes over the same 1500 real repaint frames). So the 250 ms whole-screen quiet was unobtainable for the length of a turn: 227 deliveries in 24 h waited past 2 s, mean 54 s, max 561 s — every one of them onto a composer that was empty and safe, since claude/codex queue input received mid-turn. The output question is now asked of the composer alone: `composerRegionFingerprint` (render-gate) hashes the classified region's text, row span, cursor and geometry, and a write requires that fingerprint unchanged across two samples ≥ `SETTLE_BEFORE_WRITE_MS` apart (`noteComposer`, a per-session `WeakMap`; `MAX_COMPOSER_SAMPLE_GAP_MS` = 2 s bounds how long an observation may go unrefreshed before the clock restarts, so a stale sample cannot authorise a write onto a just-repainted composer). The whole-screen settle is KEPT as an alternative proof — no output at all is strictly stronger, and it lets an idle recipient deliver on its first pass. The residual hold is named `busy:composer-redraw`, self-clearing, with a `retryAfterMs` that shares #1473's one-shot re-drain timer. **The licence is per-app**: `GateProfile.queuesInputMidTurn` gates it, and an app without it keeps the #1573 whole-screen settle end to end (pre-lock and in-lock alike), so a newly added profile is conservative until granted. All three shipped profiles carry it, from two different kinds of authority, and the difference is worth keeping straight. **claude 2.1.266 and codex 0.153.4 are MEASURED** true by the acceptance harness (`scripts/bugfix-1664-streaming-delivery-harness.mts`, evidence in `codev/evidence/1664-streaming-delivery/`): 100 and 20 delivering trials respectively, 0 head-loss, every message submitted. **agy is set by OWNER RULING (2026-09-09) and is UNMEASURED** — the owner's call is that mid-turn delivery is universal and only human input holds mail. The failure mode that ruling accepts: if agy drops or mishandles input arriving mid-turn, the body strands in its composer unsent, the classifier then reads it as `user-text`, and every later message to that agent queues behind it until a human clears the line — a stuck agent with a visible cause rather than silent loss, but not ruled out by anything here. Settling it is a harness run (`--harness agy --scenario streaming --assume-queues-input`), not an argument. The fingerprint covers the region's text, row span, cursor, geometry AND its per-cell SGR attributes — the last because the synchronous in-lock precheck compares fingerprints without re-classifying, so a dim placeholder becoming normal-intensity typed text of the same string would otherwise compare equal. **The order in `mailbox-delivery.ts` is load-bearing**: classify → composer stability → **input re-check** → **input settle**. A `!verdict.clean` return precedes the input settle, so a *non-empty* composer can only ever produce `busy:user-text`, never `busy:recent-input` — the two verdicts describe different guards and are not interchangeable. A race detected *during* the paced write is **reported** (`racedByInput` → `unverifiedCause`) and never retried; re-writing a message that already landed is the #1584 re-injection failure. `MailboxDrainer.noteOutcome()` exists because `tower-routes.ts` calls `deliverAgentMailSerialized` directly on the `afx send` request path, outside the drainer, so the re-drain must be armed from there too.

2c. **Terminal replies are stripped from the input SIGNAL, never from the PTY (`terminal-replies.ts`).** xterm forwards DA/DSR/CPR/XTWINOPS/DECRPM/DECRQSS/OSC-colour/focus **replies** upstream through the same path as keystrokes. Counting them as input would hold mail with nobody at the keyboard, and would **self-trip**: delivery repaints the TUI → the TUI queries the client → the client answers → the answer reads as typing. The filter is server-side so it covers every client, not one cooperative one (`apps/web`'s own stripping is partial and only during `rc.initialPhase`). It is **signal-only** — the PTY still receives every byte verbatim, because applications block waiting on their own DA/DSR answers. Both failure directions are silent: an under-strip holds mail with nobody present; an over-strip stops counting a real keystroke and silently re-opens the race while every gate test still passes. The reply table is therefore **pinned to the installed `@xterm/xterm` version by a test** — a bump that adds a newly-answered query (kitty keyboard, XTVERSION) turns into an uncounted-reply hold and must not pass silently. `AF_LOG_INPUT_SIGNAL=1` prints, per external write, the raw chunk, what was stripped, and what survived; `survived=<NOTHING>` is the hands-off marker.

3. **Honest response vocabulary.** `delivered` (gate passed, write completed) or `held` + row id + **why-held reason** ∈ {`busy` (draft/menu/mode), `no-profile` (unknown app), `no-live-pty` (no live terminal)}. Additive over the old shape (`ok`/`terminalId`/`deferred` retained for old binaries; a `held` outcome is still `ok:true`). Surfaced to senders through `packages/core/src/tower-client.ts` and `commands/send.ts` (both single-send and `--all` aggregation). **The reason alone is not a diagnosis (#1482).** `busy` conflates two opposite situations: a human at the line (`user-text`, `recent-input`, and — since #1664 — `composer-redraw` for a composer still being painted; all self-clearing) and a classifier that could not verify anything (`no-region-end` / `no-composer-marker` / `no-profile` — the hold never clears without intervention). `GateVerdict.detail` used to die inside `DeliveryOutcome`, so every operator surface showed a bare `busy` for both and offered the same remedy. Migration **v18** adds a nullable `mailbox.detail` column (PRAGMA-gated, deliberately with no CHECK constraint — SQLite cannot `ALTER` one in, and declaring it only in `GLOBAL_SCHEMA` would make a fresh install structurally differ from an upgraded one, which the convergence test forbids; the `MailboxGateDetail` type is the enforcement). `setHeldVerdict` writes reason+detail in one statement, and its own SQL carries the changed-only predicate (`AND (reason IS NOT ? OR detail IS NOT ?)` — `IS NOT`, not `<>`, because both columns are nullable and `NULL <> NULL` is NULL rather than false), so `updated_at` keeps meaning "when the verdict last moved" for every caller, not only the delivery pass that also guards at its two call sites; non-gate holds write `detail: null` so a stale detail cannot outlive its verdict. It surfaces as a `reason:detail` sub-code on `afx inbox`, `afx inbox show`, the send response, the cron held log, the escalation log and its SSE payload. `isUnverifiableVerdict` (`utils/hold-verdict.ts`) is the single predicate splitting the two classes — the owner starvation notice branches on it to avoid suggesting `afx interrupt` against a human mid-draft.
...
`formatMessageForTarget` (`tower-routes.ts`, via `utils/message-format.ts`) is the single funnel: it wraps a delivered body in a header keyed by target + `from`. There are three classes. A message **to a builder** renders as `[ARCHITECT INSTRUCTION]`, or `[ARCHITECT:<name> INSTRUCTION]` when `from` is an `architect:<name>` identity (#1478). A message **to an architect with a `from`** takes its label from the sender's SHAPE, not from a hardcoded role — `[BUILDER <id> MESSAGE]` for a builder, `[ARCHITECT:<name> MESSAGE]` for architect → architect. And (#1494) a message **to an architect with `from === VSCODE_USER_SENDER`** (the `'vscode-user'` wire constant in `@cluesmith/codev-types`) renders as `[USER via VS Code]`. That third class exists so a human's VS Code action relayed to the architect is distinguishable from a peer-architect instruction. The VS Code Approve-gate button sends its relay with that `from`; the extension itself never runs `porch approve`. Under the builder-runs-it convention the architect passes the relay to the builder, which runs the command in its own worktree.

**Naming the architect is what makes the header answer its question (#1478).** `afx send` carries the specific architect as `architect:<name>` (`architectSenderId()`, shared verbatim by `afx interrupt` and `afx refresh`) instead of collapsing every architect to the generic string `architect`; the mailbox row's `from_agent` is the carrier, so `afx inbox` and the dashboard inherit the same identity from the shared store. Two rules keep the label honest, both enforced in `message-format.ts` rather than at call sites. First, **role follows the sender's prefix, not the validity of its name**: an architect-shaped sender whose name does not validate (`architect:Main`, the empty `architect:`, the default auto-numbered `architect-3`) degrades to the bare `ARCHITECT`, never to `BUILDER architect-3` — a wrong role on a real identity is the defect the label exists to prevent, and the prefix test mirrors `looksLikeBuilderId`, which excludes `architect*` for the same reason. Second, **nothing reaches the framing unvalidated**: the architect branch validates the name against `ARCHITECT_NAME_PATTERN`, the builder branch against `SAFE_SENDER_ID`, so an id that merely looks architect-shaped cannot forge a `### [...] ###` block; an unshowable one degrades to `BUILDER <unknown>`. `POST /api/send` refuses a `from` failing that same predicate (`isSafeSenderId`, `MAX_SENDER_ID_LENGTH` = 128) before the row is persisted — the sender is otherwise the one unbounded field on a public local route, and `afx inbox` sizes its `FROM → TO` column to the widest stored value, so an unbounded identity would be a rendering cost paid by every row.

**Every wrapper is self-attesting (#1574).** The header also names the RECIPIENT — `[BUILDER <sender> MESSAGE → <to_agent> | <ts>]`, and likewise for the other two variants — where `to_agent` is the canonical identity the mailbox row is keyed on. Without it no pane-reader could verify a frame on its screen was addressed to it: a genuine misdelivery was invisible to its victim (the one party positioned to detect it), and a false claim of one could not be refuted from the screen. `toAgent` is a required formatter argument, so the recipient is threaded from resolution rather than defaulted. Builder-bound frames additionally carry `(reply: afx send architect "…")` **on the closing delimiter line** (`formatArchitectToBuilderMessage`) — the reply channel stated at the point of need, because a builder that types a reply as assistant text in its own terminal reaches nobody and gets no feedback (the real defect behind #1530). The same line, not a fourth one, is deliberate: `message-write.ts` sends frames of `BRACKET_MIN_LINES` (4) lines or more — or over `BRACKET_MIN_BYTES` — as a bracketed, chunked paste (#1567), so a trailing line would move every short architect→builder message off the single-write path as a side effect of a formatter change. `--raw` sends are unwrapped and gain neither.

**The limit (do not build on this header as proof):** `from` is caller-controlled, so within Tower's keyed boundary (post-#1421 every route accepting this message requires the shared local key) the header **differentiates** senders for triage, it does **not** prove provenance. This applies to **both** attributed classes, for the same reason: a `[USER via VS Code]` header does not prove a human click, and an `[ARCHITECT:<name>]` header does not prove that architect sent it — the name is validated for *shape*, which bounds what can be rendered, not for *authenticity*. Naming the sender is the #1094 header-laundering concern in miniature: a recipient must not treat a header as authorization for what the body asks. A gate-spending or bypass-spending decision still requires the standing provenance bars (the human's word in the executing channel, or evidence independent of the requesting chain), never a header alone.
...
### 8. Request Authentication in Tower: the Shared Local Key
**Decision**: Every non-public Tower route — HTTP and WebSocket upgrades alike — requires the shared local key (`~/.agent-farm/local-key`, generated by Tower at boot, mode 0600), compared in constant time and failing closed (advisory GHSA-xvjp-7748-v88v, PR #1421, merged 2026-08-18).

**How the key travels**:
- **CLI / SDK / VS Code extension** read the file directly (possession proves same-OS-user) and present it as the `codev-tower-key` header, or a `codev-key.<KEY>` WebSocket subprotocol token
- **Browser pages** cannot read files, so Tower injects `window.__CODEV_TOWER_KEY__` into the HTML shells it serves same-origin; those shells strip `Access-Control-Allow-Origin` so a cross-origin fetch can never read the key, and a Host allowlist blocks DNS rebinding
- **Public allowlist is deliberately narrow**, and every entry exists because a *browser navigation* reaches it and cannot attach a header: `/health`, `/api/version`, the dashboard shell and its static assets, the annotator shell + vendor files (iframe / `<script>` / `<link>` loads), and `GET /api/tunnel/connect/callback` — the cloud OAuth redirect, which is nonce-authenticated instead (single-use, 5-min TTL, mintable only by an authenticated `POST /api/tunnel/connect`; #1570). Everything else under `api/` and `ws/` requires the key. `BRIDGE_MODE` refuses to start without a key at all
- **Reviewing a carve-out** means enumerating the header-less entry points, not just walking the API surface: #1421's 3-way consult and architect review both walked the routes and still missed the OAuth redirect, 401ing every fresh cloud registration until #1570
