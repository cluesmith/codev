...
2. **Gate before every write.** The render-gate (`render-gate.ts`) replays the session's seed-capped output ring through `@xterm/headless` and applies a per-app classifier profile (`gate-profiles.ts`: **claude** & **codex** use a dim-placeholder rule; **agy** uses a color-keyed `placeholderFgPalette` rule — see below). Clean (marker present AND composer region empty) → deliver; else → keep holding.
