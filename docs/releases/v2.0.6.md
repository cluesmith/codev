...
- **Reworked `afx spawn` CLI**: Positional arg + `--protocol` flag replaces old `-p`/`--issue` syntax
...
- **`afx attach` terminal mode**: Direct shellper socket connection for terminal access
...
- **`afx send` command**: Send messages between architect and builder agents
