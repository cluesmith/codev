...
- **Keyboard forward of the symbol or hunk at the cursor** (#1073, PR #1447). `Cmd/Ctrl+K H` in a builder diff forwards whatever covers the cursor to that builder's terminal — the enclosing symbol first (resolving to exactly the range its codelens would send), else the containing changed hunk, else the bare file path with a status-bar note. No selection required and no Enter pressed, so the reference lands and you keep typing. Gives the mouse-only codelens a keyboard equivalent; `Cmd/Ctrl+K B` and the lens itself are untouched.
...
- **Stream Deck: the builder row sizes itself to the keys you place** (#1465, PR #1468). The Row 1 selector window used to page the fleet four at a time no matter how many Builder Action keys were on the board, so placing three (to free a slot for another key) left every fourth builder reachable by the Select dial but shown on no key — selected, driving the action row and both dials, with nothing lit to say so. The window now matches however many selector keys you have placed, on any deck size, and the keys order themselves left to right and top row first, so the manual slot number in the key's settings is gone. This is what lets the recommended layout put **Open Architect Terminal** in the first slot with the builder window filling the rest.
- **Stream Deck: the Scroll dial says what it will do** (#1498, PR #1504). It used to show one static word while the three dials beside it described themselves — and it was the dial whose press behaviour actually varies, since a press either sends your selection to the builder immediately or stages it in the review queue depending on the workspace's delivery mode. Its touchstrip now names that live mode and the builder the press acts on, reads "No builder" when nothing is selected, and reads "Scroll · editor only" while you are reviewing a spec or plan, where the dial genuinely cannot act. No progress bar: a bar under a dial that scrolls a viewport would suggest a relationship that does not exist.
- **Stream Deck: an architects board, alongside the builders board** (#1495, PR #1499). A new **Architect Action** key gives each architect a tile of its own: place several and they order themselves left to right and top row first, each showing a live architect's name, each opening that architect's terminal on press. Put them on a second page or profile and reach them with Stream Deck's own **Switch Profile** or **Folder** key — the plugin ships matching artwork for it and the README carries the wiring steps — so the deck flips between the high-level view of who is working and the builder-level view of what they are doing. The list follows the architects the workspace currently has, so an architect that is not running simply is not there.
- **Stream Deck: open an architect's terminal from the deck** (#1463, PR #1466). A new **Open Architect Terminal** key reaches the other half of the fleet: by default it opens the terminal of the architect that spawned the builder you have selected, so escalating from a review is one press rather than a trip to the mouse; set to Main instead, it always opens your main architect. The key names its target on its face before you press, and dims to "None" when there is nothing to open rather than looking live and doing nothing. The existing Open Terminal key is renamed **Open Builder Terminal** so the pair reads as a set — **keys you have already placed keep working and simply relabel themselves**, so there is nothing to re-place.
- **Stream Deck: the generic Codev Action key wears the Codev mark** (#1444, PR #1453). It previously drew a terminal picture, which became confusing once Open Terminal got its own (correctly terminal-shaped) icon. Because this key has no live face of its own, the change shows both in the Stream Deck app's action list and on the physical key.
- **Stream Deck: Send Feedback and Open Terminal get their own icons** (#1440, PR #1443). The two newest actions no longer borrow the generic Codev Action glyph in the Stream Deck app's action list, so they are distinguishable when you place keys. The icons are rendered from the same glyph vectors the live key faces draw, so the picker and the hardware always agree. Six unused icon files left over from earlier key work are gone.
