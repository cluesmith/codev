...
A minor release that turns `.builders/<id>/` worktrees into **runnable, reviewable environments** — from both the CLI and the VS Code sidebar — and aligns the workspace on a single lockstep version. Almost all of this release is the work of Amr Elsayed (@amrmelsayed).
...
```bash
npm install -g @cluesmith/codev@3.1.0
