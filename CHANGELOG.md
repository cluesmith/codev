...
- **Built-in `gemini` harness retired** (breaking change): the standalone **Gemini CLI** is no longer a supported built-in builder or architect shell. Google ended consumer Gemini CLI access (Pro, Ultra, and free tiers) on 2026-06-18. A `gemini` shell now **fails closed** at spawn/launch with a retirement message (no orphaned worktree, no Tower crash), and `codev doctor` flags a persisted `gemini` builder/architect config. **Migration**: set `shell.architect` / `shell.builder` to a supported harness — `claude` or `codex` (either role), or `opencode` (builder only; OpenCode is not a supported architect shell). If you retain Gemini access (Standard/Enterprise or API-key), define a **custom harness** named `gemini` in `.codev/config.json` **and select it explicitly** via `shell.builderHarness` / `shell.architectHarness` — a bare auto-detected `gemini` command stays retired. See the README's custom-harness escape hatch for a working config snippet (Gemini injects the role via the `GEMINI_SYSTEM_MD` env var, not a `--system` flag). This does **not** affect the `gemini` **consult lane**, which uses the Antigravity CLI `agy`.
